...
export async function POST(
  ...
  try {
    ...
    if (dbSession.session_type === 'single' || dbSession.session_type === 'mocap') {
      // For single/mocap sessions, we cannot add additional participants
      // This is a 1:1 session and adding more would require converting to a group session
      return NextResponse.json(
        { error: 'Cannot add additional participants to a 1:1 session. This session type only supports one participant.' },
        { status: 400 }
      );
    } else {
      // For group sessions, update subject_ids array by adding new participants
      // This preserves existing participants and adds new ones
      console.log('[API] Updating group session - existing:', existingSubjectIds, 'new:', newSubjectIds, 'updated:', updatedSubjectIds);
      ...
      console.log('[API] Successfully updated session with new participants. Total participants:', updatedSubjectIds.length);
    }
